  ...
  */
  ...
  WiFiClient client;
  const char* ssid = "cy";
  const char* password = "11011059590";
  ...
  void loop() {
    ...
    if (tmp < 10) {
    setBuff(0x20, 0x20, 0x20);// White color for temperature below 10 degrees
    ...
    Serial.print("Hum: ");
      Serial.println(hum);  // Print temperature for debugging purposes
      if (hum < 50) {
          delay(500); // Turn off LED for 0.5s
        M5.dis.displaybuff(DisBuff);
      } else if (hum >= 50 && hum < 60) {
          delay(1000); // Turn off LED for 1s
         M5.dis.displaybuff(DisBuff);
      } else {
         M5.dis.displaybuff(DisBuff);
      }
