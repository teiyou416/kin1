...
for (int i = 1; i < client_count+1; ++i) {
        printf("Client %d:IP:%s  Port %d\n", i , inet_ntoa(clients[i].address), clients[i].port);
    }
  ...
  if ((c_pid = wait(&status)) == -1) {
    ...
    if ((c_pid = waitpid(-1, &status, WNOHANG)) > 0) {
        for (int i = 0; i < client_count; ++i) {
            if (clients[i].active && clients[i].pid == c_pid) {
                clients[i].active = 0; 
...
++client_count;
 if (client_count < MAX_CLIENTS) {
            memcpy(&clients[client_count].address, &cl_addr.sin_addr, sizeof(struct in_addr));
             clients[client_count].active = 1;
            clients[client_count].port = ntohs(cl_addr.sin_port);
        }
pid_t pid =fork();
if(pid<0){
perror("fork");
